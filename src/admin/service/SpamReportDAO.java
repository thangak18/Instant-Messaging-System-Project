...
public class SpamReportDAO {
    ...
    public List<SpamReport> getAllSpamReports() throws SQLException {
        ...
        String sql = "SELECT sr.report_id, sr.reporter_id, sr.reported_user_id, sr.reason, " +
                    ...
                    "JOIN users u1 ON sr.reporter_id = u1.user_id " +
                    "JOIN users u2 ON sr.reported_user_id = u2.user_id " +
    ...
    public List<SpamReport> searchSpamReports(String searchType, String keyword, 
        ...
        StringBuilder sql = new StringBuilder(
            ...
            "u2.full_name as reported_user_name, u2.username as reported_username " +
            "FROM spam_reports sr " +
            "JOIN users u1 ON sr.reporter_id = u1.user_id " +
            "JOIN users u2 ON sr.reported_user_id = u2.user_id WHERE 1=1");
        ...
        if (status != null && !"Tất cả trạng thái".equals(status)) {
            String statusValue;
            switch (status) {
                case "Chờ xử lý": statusValue = "pending"; break;
                case "Đã xử lý": statusValue = "resolved"; break;
                case "Từ chối": statusValue = "rejected"; break;
                default: statusValue = status.toLowerCase();
            }
